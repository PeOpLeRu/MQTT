...
DELTA = 3
THRESHOLD = 5 # Количество последних значений, для которых учитывается порог освещенности
queue = [0]
min_value = 100
max_value = 0
...
def action_1(client, data, message):
    ...
    if len(queue) >= 4: # Последние 4 значения влияют на тенденцию текущего участка измерений
        queue.pop()
    ...
    if data < d_avg_min:  # Если текущее значение меньше последних нескольких значений, то тенденция яркости падает и светодиод включается
        ser.write("1".encode())
    elif data > d_avg_max:
        ser.write("0".encode())
...
if mode == 1:
    ...
    raise("Выбран неправильный режим работы!")
